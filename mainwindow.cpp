    ...
    , COMPORT(nullptr)
{
    ...
    // --- UI Stil Ayarları (Buton Renkleri) ---
    QString greenStyle = "background-color: green; color: white; border: 0.5px solid black;";
    QString redStyle   = "background-color: red; color: white; border: 0.5px solid black;";

    ui->pushButton_2->setStyleSheet(greenStyle);
    ui->pushButton_6->setStyleSheet(greenStyle);
    ui->pushButton_7->setStyleSheet(greenStyle);
    ui->pushButton_8->setStyleSheet(greenStyle);
    ui->pushButton_9->setStyleSheet(redStyle);
    ui->pushButton_10->setStyleSheet(greenStyle);
    ...
    ui->sourceCam->setFixedSize(150, 32); // Genişlik düzeltildi
    ...
    // --- GRAPH 1: Yükseklik ---
    ui->graph1->addGraph(); // 0: Altitude1
    ui->graph1->addGraph(); // 1: Altitude2
    ui->graph1->addGraph(); // 2: AltitudeDiff
    ...
    // --- GRAPH 4: Basınç ---
    ui->graph4->addGraph(); // 0: Pressure1
    ui->graph4->addGraph(); // 1: Pressure2
    ...
    ui->graph4->graph(1)->setPen(yellow);
    ...
    // Grafiklerin font ayarları (Toplu)
    QList<QCustomPlot*> plots = {ui->graph1, ui->graph2, ui->graph3, ui->graph4, ui->graph5};
    for(auto plot : plots) {
...
void MainWindow::updateFrame()
{
    ...
    QString errorCode = List[2].trimmed();
    ARAS(errorCode);
...
void MainWindow::recalibrateGraphs(){
    ...
    x.clear();
    yAlt1.clear(); yAlt2.clear(); yAltDiff.clear();
    yTemp.clear(); yPressure1.clear(); yPressure2.clear();
    yVoltage.clear(); yDescent.clear();
